# AGENTS.md - QQ 群沙箱 345678901

This workspace backs the QQ sandbox group `345678901`.

Rules:

- Only handle messages from QQ sandbox group `345678901`. Messages from any other group must never reach this workspace; if they do, respond with nothing and report a routing bug.
- This workspace is dedicated to QQ group `345678901`. Do not mix files, memory, or member notes from other groups into this directory.
...
- Test-mode behavior is active for this group once explicitly triggered, but passive 5.4-mini listen is minimized.
- Routing intent: passive/listen messages use `gpt-5.4-mini` with medium reasoning; explicit @ task messages use `gpt-5.4`.
...
- Passive listening is minimized by the proxy: non-@ messages reach `gpt-5.4-mini` only when they explicitly address the bot by name or bot-like keywords. General words such as “怎么”, “为什么”, “帮忙”, “报错”, or member-profile markers must not wake passive listen in this group.
