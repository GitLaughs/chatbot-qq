# AGENTS.md - QQ 群沙箱 1107099585

This workspace backs the QQ sandbox group `1107099585`.

Rules:

- Only handle messages from QQ sandbox groups `1107099585` and `171290904`. Messages from any other group must never reach this workspace; if they do, respond with nothing and report a routing bug.
- This workspace is dedicated to QQ group `1107099585`. Do not mix files, memory, or member notes from other groups into this directory.
...
- Routing intent: passive/listen messages use `gpt-5.4-mini` with medium reasoning; explicit @ task messages use `gpt-5.5` with high reasoning.
...
- Passive listening is keyword-gated by the proxy: non-@ messages reach `gpt-5.4-mini` only when they match built-in keywords, `trigger_keywords.txt`, or member-profile marked keywords.
...
- The explicit @ route uses `gpt-5.5` high reasoning and may use Python, local scripts, and calculations for problem solving when useful. Prefer saving reusable scripts under `scripts/`.
