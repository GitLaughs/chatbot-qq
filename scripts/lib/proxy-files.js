...
function createProxyFiles(deps) {
  ...
  function saveGroupFileData(workspace, pendingInfo, data) {
    ...
    try {
      ...
      if (deps.recordError) {
        deps.recordError("file-save", err.message, { scope: "group", target: String(pendingInfo.groupID), detail: name });
      }
      return null;
    }
  }
  ...
  async function archiveSavedFile(workspace, saved, pendingInfo) {
    ...
    if (extracted) {
      ...
      fs.writeFileSync(extractedPath, clean, "utf8");
      fs.writeFileSync(summaryPath, deps.buildFileSummary(saved, clean, meta), "utf8");
      addFileIndex({
        workspace,
        scope: "group",
        scopeID: String(pendingInfo.groupID),
        userID: String(pendingInfo.userID || ""),
        messageID: String(pendingInfo.messageID || ""),
        name: saved.name,
        originalName: pendingInfo.fileName,
